...
export const OptionsScreen: React.FC<Props> = ({ 
  ...
  return (
    <div className="h-full bg-slate-900 flex flex-col overflow-y-auto">
      <div className="p-6 border-b border-slate-800 flex items-center gap-4 sticky top-0 bg-slate-900/90 backdrop-blur z-10">
        <button onClick={onBack} className="p-2 hover:bg-slate-800 rounded-full text-slate-400 hover:text-white">
          <ArrowLeft className="w-6 h-6" />
        </button>
        <h1 className="text-xl font-bold text-white">Opciones</h1>
      </div>
      ...
      <div className="p-6 max-w-2xl mx-auto w-full space-y-8 pb-10">
        ...
        <section className="space-y-4">
          <h2 className="text-sm uppercase tracking-wider text-slate-500 font-semibold">Sonido de Alarma</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className={`p-4 rounded-xl border flex flex-col gap-3 transition-all ${
                ...
                >
                  ...
                  {settings.sound === 'custom' && <Bell className="w-5 h-5 text-sky-400 fill-current" />}
                </button>
                ...
                <div className="flex items-center gap-2">
                   ...
                   <button 
                      ...
                      className="flex-1 flex items-center justify-center gap-2 bg-slate-900 py-2 rounded-lg text-xs font-medium text-slate-400 hover:text-white border border-slate-700 hover:border-slate-500 transition-colors"
                   >
          ...
          <button 
            ...
            className="flex items-center gap-2 text-sky-400 hover:text-sky-300 text-sm font-medium mt-2 px-2"
          >
        ...
        {/* Repetitions */}
        <section className="space-y-4">
           <h2 className="text-sm uppercase tracking-wider text-slate-500 font-semibold">Configuración de Repeticiones</h2>
           <div className="bg-slate-800 p-6 rounded-xl border border-slate-700">
             <div className="flex items-center justify-between mb-2">
               <span className="text-white">Cantidad</span>
               <span className="text-sky-400 font-mono text-xl">{settings.repetitions}</span>
             </div>
             <input 
               type="range" 
               min="1" 
        ...
        {/* Interval */}
        <section className="space-y-4">
           <h2 className="text-sm uppercase tracking-wider text-slate-500 font-semibold">Intervalo (ms)</h2>
           <div className="bg-slate-800 p-6 rounded-xl border border-slate-700">
             <div className="flex items-center justify-between mb-2">
               <span className="text-white">Velocidad</span>
               <span className="text-sky-400 font-mono text-xl">{settings.interval}ms</span>
             </div>
             <input 
               type="range" 
               min="100" 
